...
module.exports = {
  index: async (req, res) => {
    try {
      const value = await db.freight_forworders.findAll()
  ...
  store: async (req, res) => {
    ...
    if (data) {
      try {
        const oldName = await db.freight_forworders.findOne({
        ...
        const value = await db.sequelize.transaction((t) => {
          return db.freight_forworders.create(data, { transaction: t })
        })
  ...
  update: async (req, res) => {
    ...
    const data = req.body
    if (id && data) {
      const result = await db.freight_forworders.update(data, { where: { id: id } })
  ...
  destroy: async (req, res) => {
    ...
    if (id) {
      try {
        await db.freight_forworders.destroy({ where: { id } })
